...
-(void)handleRefresh {
    ...
    for (Ride *ride in [self ridesForCurrentIndex]) {
        if ([ride.updatedAt compare:lastUpdate] == NSOrderedDescending) {
            lastUpdate = ride.updatedAt;
        }
    }
    
    [[RidesStore sharedStore] fetchRidesfromDate:lastUpdate rideType: -1 block:^(BOOL fetched) {
        if (fetched) {
            [[RidesStore sharedStore] initAllRidesFromCoreDataWithBlock :^(BOOL fetched) {
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([ride.rideType intValue] == ContentTypeCampusRides) {
        cell.rideTypeLabel.text = @"Campus";
        cell.rideTypeImageView.image = [UIImage imageNamed:@"CampusIcon"];
    } else {
        cell.rideTypeLabel.text = @"Activity";
        cell.rideTypeImageView.image = [UIImage imageNamed:@"ActivityIcon"];
    }
    ...
    if ([ride.freeSeatsCurrent intValue] == 0) {
        cell.seatsView.backgroundColor = [UIColor grayColor];
    } else {
        cell.seatsView.backgroundColor = [UIColor orangeColor];
    }
    if ([ride.isRideRequest boolValue]) {
        cell.seatsView.hidden = YES;
	} else if ([ride.freeSeatsCurrent intValue] == 1) {
        cell.seatsView.hidden = NO;
        cell.seatsLabel.text = @"1 seat free";
    } else {
		cell.seatsLabel.text = [NSString stringWithFormat:@"%d seats free", (int)([ride.freeSeatsCurrent intValue])];
    }
...
-(NSArray *)ridesForCurrentIndex {
    ...
    switch (self.index) {
        case 0:
            return [[RidesStore sharedStore] allRides];
        case 1:
            campus = [[RidesStore sharedStore] ridesNearbyByType:0];
            activity = [[RidesStore sharedStore] ridesNearbyByType:1];
            sortedArray = [[campus arrayByAddingObjectsFromArray:activity] sortedArrayUsingComparator:^NSComparisonResult(id a, id b) {
            ...
            return sortedArray;
        case 2:
            campus = [[RidesStore sharedStore] favoriteRidesByType:0];
            activity = [[RidesStore sharedStore] favoriteRidesByType:1];
            sortedArray = [[campus arrayByAddingObjectsFromArray:activity] sortedArrayUsingComparator:^NSComparisonResult(id a, id b) {
                NSDate *first = [a departureTime];
                NSDate *second = [b departureTime];
                return [first compare:second] == NSOrderedDescending;
