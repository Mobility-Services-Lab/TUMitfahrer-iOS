...
fetchCompletionHandler:(void (^)(UIBackgroundFetchResult))handler {
    ...
    if (application.applicationState == UIApplicationStateActive ) {
        ...
        if ([[alert valueForKey:@"title"]  isEqual: @"TUMitfahrer : A ride has been canceled"]) {
            alertView.tag = 0;
        } else {
        ...
        NSLog(@"app reactivated via notification");
        NSDictionary *aps = [userInfo valueForKey:@"aps"];
        NSDictionary *alert = [aps valueForKey:@"alert"];
        if ([[alert valueForKey:@"title"]  isEqual: @"TUMitfahrer : A ride has been canceled"]) {
            [self setupNavigationControllerWithRide:0];
            return;
        }
...
- (void)alertView:(UIAlertView *)alertView didDismissWithButtonIndex:(NSInteger)buttonIndex{
    ...
    if (buttonIndex == 0){
        ...
        if (alertView.tag == 0) {
            NSLog(@"Ride cancelation notification");
            return;
        }
...
-(void)setupNavigationControllerWithRide:(NSNumber*)rideId {
    ...
    RidesPageViewController *parentVC = [[RidesPageViewController alloc] init];
    navControler = [[UINavigationController alloc] initWithRootViewController:parentVC];
    ...
    if(rideId==nil || [rideId isEqual:[NSNumber numberWithInt:0]]){//invalid rideID or cancelation notification
        return;
    }
