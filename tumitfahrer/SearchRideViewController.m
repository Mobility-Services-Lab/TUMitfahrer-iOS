...
#import "SearchSegmentedControlCell.h"
#import "SliderCell.h"
#import "ButtonCell.h"
...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil {
    ...
    if (self) {
        self.tableValues = [[NSMutableArray alloc] initWithObjects:@"", @"", @"1", @"", @"1", @"", @"", @"", nil];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row == 0) {
        SearchSegmentedControlCell *cell = [SearchSegmentedControlCell segmentedControlCell];
        cell.delegate = self;
        [cell setFirstSegmentTitle:@"All" secondSegmentTitle:@"Campus" thirdSegmentTitle:@"Activity"];
        [cell addHandlerToSegmentedControl];
        cell.controlId = 0;
        
#ifdef DEBUG
        // add label for kif test
        [[[[cell segmentedControl] subviews] objectAtIndex:0] setAccessibilityLabel:@"All"];
        [[[[cell segmentedControl] subviews] objectAtIndex:1] setAccessibilityLabel:@"Activity"];
        [[[[cell segmentedControl] subviews] objectAtIndex:2] setAccessibilityLabel:@"Campus"];
        ...
        [[[[cell segmentedControl] subviews] objectAtIndex:2] setIsAccessibilityElement:YES];
#endif
        return cell;
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if(indexPath.row == 1 || indexPath.row == 3) {
        ...
        destinationVC.predefinedDestinations = [NSMutableArray arrayWithObjects:@"Anywhere", @"Arcistraße 21, München", @"Garching-Hochbrück", @"Garching Boltzmannstraße", nil];
...
-(void)buttonSelected {
    ...
    if ((departurePlace == nil || departurePlace.length == 0) && (destination == nil || destination.length == 0) ) {
        [ActionManager showAlertViewWithTitle:@"Invalid search" description:@"Please specify departure and/or destination place"];
	...
	if (departurePlace.length != 0) {
		[queryParams setValue:[NSNumber numberWithDouble:self.departureCoordinate.latitude] forKey:@"departure_latitude"];
		[queryParams setValue:[NSNumber numberWithDouble:self.departureCoordinate.longitude] forKey:@"departure_longitude"];
	...
	if (destination.length != 0) {
		[queryParams setValue:[NSNumber numberWithDouble:self.destinationCoordinate.latitude] forKey:@"destination_latitude"];
		[queryParams setValue:[NSNumber numberWithDouble:self.destinationCoordinate.longitude] forKey:@"destination_longitude"];
		[queryParams setValue:destinationThreshold forKey:@"destination_threshold"];
	}
    
    if (self.searchType != 0) { //if all rides, then no "ride_type" parameter will return all rides
        [queryParams setValue:[NSNumber numberWithInteger: self.searchType - 1] forKey:@"ride_type"];
    }
