...
- (IBAction)registerButtonPressed:(id)sender {
    ...
    // add enum
    queryParams = @{@"email": self.emailTextField.text, @"first_name": self.firstNameTextField.text, @"last_name":self.lastNameTextField.text, @"department": [[FacultyManager sharedInstance] nameForFacultyFriendlyName:self.departmentNameTextField.text]};
...
//        }
        ...
        if(operation.HTTPRequestOperation.response.statusCode ==400){
            [ActionManager showAlertViewWithTitle:@"Error" description: @"Not a valid mail address. Use one of these: tum.de, *.tum.de, cs.tum.edu, mytum.de" ];
        } else {
...
-(UIPickerView *)preparePickerView {
    ...
    [self.pickerView selectRow:[[FacultyManager sharedInstance] indexForFacultyFriendlyName:self.departmentNameTextField.text] inComponent:0 animated:NO];
...
-(void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component {
    self.departmentNameTextField.text = [[FacultyManager sharedInstance] friendlyNameOfFacultyAtIndex:[self.pickerView selectedRowInComponent:0]];
}
...
-(void)customIOS7dialogButtonTouchUpInside:(id)alertView clickedButtonAtIndex:(NSInteger)buttonIndex {
    if (buttonIndex == 0) {
        if ([self.departmentNameTextField.text isEqualToString:@""]) {
            self.departmentNameTextField.text = [[FacultyManager sharedInstance] friendlyNameOfFacultyAtIndex:0];
        }
    }
    [alertView close];
}
