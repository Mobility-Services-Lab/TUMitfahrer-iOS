...
-(void)initAllRidesFromCoreDataWithBlock:(boolCompletionHandler)block {
    [self loadAllRidesFromCoreData];
    [self filterAllRides]; // categorize rides to rides around me and my rides
    [self checkPhotoForEachRide];
    block(YES);
...
-(void)fetchNewRides:(boolCompletionHandler)block {
	...
	UIAlertView *waitAlert = [[UIAlertView alloc] initWithTitle:@"Loading" message:nil delegate:self cancelButtonTitle:nil otherButtonTitles: nil];
	
	UIActivityIndicatorView *indicator = [[UIActivityIndicatorView alloc] initWithActivityIndicatorStyle:UIActivityIndicatorViewStyleGray];
	[indicator startAnimating];
	
	[waitAlert setValue:indicator forKey:@"accessoryView"];
	[waitAlert show];
    RKObjectManager *objectManager = [RKObjectManager sharedManager];
    [objectManager.HTTPClient setDefaultHeader:@"Authorization" value:[CurrentUser sharedInstance].user.apiKey];
...
-(void)fetchRidesfromDate:(NSDate *)date rideType:(NSInteger)rideType block:(boolCompletionHandler)block {
    ...
    if (rideType == -1) {
        queryParams = @{@"from_date": date};
    } else {
        queryParams = @{@"from_date": date, @"ride_type" : [NSNumber numberWithInt:(int)rideType]};
    }
