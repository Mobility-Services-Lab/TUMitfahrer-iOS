...
-(void)viewWillAppear:(BOOL)animated {
    ...
    self.headerTitles = [NSArray arrayWithObjects:@"Details", @"Driver", @"Passengers", @"Requests", @"", nil];
    self.skippedDriver = false;
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    ...
    if (section == 2 && [self.ride.passengers count]-1 > 0) {
        return [self.ride.passengers count]-1;
    } else if(section == 3 && [self.ride.requests count] > 0) {
        return [self.ride.requests count];
    } else if(section == 4 && self.ride.regularRideId != nil) { // cancel button
        return 2;
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if(indexPath.section == 0) {
        ...
        return cell;
    } else if (indexPath.section == 2 && [self.ride.passengers count]-1 > 0) { // show passengers, one of the passengers is always the driver
        RidePersonCell *cell = [tableView dequeueReusableCellWithIdentifier:@"RidePersonCell"];
        if (cell == nil) {
            cell = [RidePersonCell ridePersonCell];
        }
        ...
        if (self.skippedDriver) {
            passenger = [[self.ride.passengers allObjects] objectAtIndex:indexPath.row + 1];
        }
        
        passenger = [[self.ride.passengers allObjects] objectAtIndex:indexPath.row];
        if ([passenger isEqual:self.ride.rideOwner]) {
            passenger = [[self.ride.passengers allObjects] objectAtIndex:indexPath.row + 1];
            self.skippedDriver = true;
        }
    ...
    } else if(indexPath.section == 2 && [self.ride.passengers count]-1 == 0) {
        ...
        return cell;
    } else if(indexPath.section == 3 && [self.ride.requests count] > 0) { // show requests
        ...
        return cell;
    } else if(indexPath.section == 3 && [self.ride.requests count] == 0) {
        EmptyCell *cell = [tableView dequeueReusableCellWithIdentifier:@"EmptyCell"];
        if (cell == nil) {
            cell = [EmptyCell emptyCell];
        }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    } else if (indexPath.section == 2 && [self.ride.passengers count] > 0) {
