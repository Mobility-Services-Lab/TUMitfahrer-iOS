...
- (void)viewDidLoad {
    ...
    self.index = 0;
    YourRidesViewController *initialViewController = [self viewControllerAtIndex:self.index];
...
-(void)viewWillAppear:(BOOL)animated {
    printf("%ld", (long)self.index);
    [self setupLeftMenuButton];
    [self setupNavigationBar];
    self.logo.titleLabel.text = [self.pageTitles objectAtIndex:self.index];
    self.logo.pageControl.currentPage = self.index;
}
...
-(void)setupNavigationBar {
    ...
    self.logo = [[LogoView alloc] initWithFrame:CGRectMake(0, 0, 200, 41) title:[self.pageTitles objectAtIndex:self.index]];
