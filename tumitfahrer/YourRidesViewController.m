...
- (void)viewDidLoad {
    ...
    NSArray *emptyCreated = [NSArray arrayWithObjects:@"No rides as driver", nil];
...
-(void)initTitlesForCurrentSection {
    if(self.index == 0) {
        self.arrayWithHeaders = [[NSMutableArray alloc] initWithObjects:@"Rides as a Driver", nil];
...
-(void)initRidesForCurrentSection {
    ...
    if (self.index == 0) {
        ...
        for (Ride *ride in [CurrentUser sharedInstance].user.ridesAsPassenger) {
            if ([ride.departureTime compare:now] == NSOrderedDescending && ![ride.rideOwner.userId isEqualToNumber:[CurrentUser sharedInstance].user.userId]) {
